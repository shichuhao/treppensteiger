...
/**
 * @brief Integrate into uphill controlMotors()
 *
 * Add after climbing mode detection:
 */
void controlMotors_withYaw() {
    // ... existing climbing logic ...

    if (isClimbingMode && stairDetected) {
        applyYawCorrection();  // Automatic base speed
        robotState = 3;
    }
    // ...
}
...
/**
 * @brief Integrate into downhill controlDownhillMotors_Downhill()
 *
 * Add before braking logic:
 */
void controlDownhillMotors_Downhill_withYaw() {
    // ... existing downhill detection ...

    // Light yaw correction during downhill (stability)
    if (shouldApplyYawCorrection()) {
        applyYawCorrection(DOWNHILL_LIGHT_BRAKE_SPEED);  // Override base speed
    }

    // ... existing braking logic ...
}

/**
 * @brief Main loop integration template
 *
 * Unified calling pattern:
 */
void mainLoopControl_withYaw() {
    if (remoteStopRequested || emergencyStop) {
        stopMotors();
        return;
    }
    ...
    if (detectDownhill_Downhill()) {
        controlDownhillMotors_Downhill_withYaw();     // Downhill + yaw
    } else if (platformNavState != NAV_IDLE) {
        executePlatformNavigation();                  // Platform nav (no yaw)
    } else {
        controlMotors_withYaw();                      // Uphill + yaw
    }
}

// ============================================================================
// END OF yaw_correction.c
// ============================================================================
