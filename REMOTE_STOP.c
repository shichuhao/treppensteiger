...
// ============================================================================
// UC01: INIT ESP8266 COMMUNICATION
// ============================================================================

/**
 * @brief Initialize ESP8266 serial communication
 *
 * @details Set up serial parameters and test connection
 * - Baud rate: 115200
 * - Data bits: 8
 * - Stop bits: 1
 * - Parity: None
...
void initESP8266() {
    ...
    Serial.println("\n╔════════════════════════════════════╗");
    Serial.println("║   ESP8266 Remote Control Init     ║");
    Serial.println("╚════════════════════════════════════╝");
    ...
    Serial.println("Ready for remote commands");
    Serial.println("Commands: STOP, START, EMERGENCY");
    Serial.println("=====================================\n");
}
...
// ============================================================================
// UC02: STOP MOTORS (L298N Control)
// ============================================================================

/**
 * @brief Stop all motors (L298N brake mode)
 *
 * @details Use L298N short-circuit braking to stop quickly
 * - Both direction pins set LOW → short-circuit braking
 * - PWM set to 0 → no drive current
 * - Stop distance < 5 cm
 *
 * L298N Truth Table (Stop Mode):
 * IN1 | IN2 | ENABLE | Result
 * LOW | LOW | LOW    |STOP
 *
 * @return void
 */
void stopMotors() {
    // Left motor group (L298N_1) brake
    digitalWrite(L298N_1_IN1, LOW);
    digitalWrite(L298N_1_IN2, LOW);
    analogWrite(L298N_1_ENA, 0);
    analogWrite(L298N_1_ENB, 0);

    // Right motor group (L298N_2) brake
