...
void executeFirstStraightMovement() {
    ...
    //Determine: Is there a staircase directly in front of ?
     ...
     if (stairsAhead) {
         ...
         platformNavState = NAV_NEXT_STAIR_CHECK;  //Go directly to the stairs and return to the stair-climbing process.
         stateStartTime = millis();
         return;
     }
...
void checkStairOrWall() {
    ...
    if (stairsAhead) {
        ...
        platformNavState = NAV_NEXT_STAIR_CHECK;  // Go directly to the stairs & climb back up the stairs
        stateStartTime = millis();
    } else {
