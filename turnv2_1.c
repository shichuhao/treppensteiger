...
void loop() {
    ...
    if (currentTime - lastSensorUpdate >= SENSOR_UPDATE_INTERVAL) {
        ...
        if (detectDownhill_Downhill()) {
            ...
            //checkNoStairCondition();//Written in function controlMotor{}
        }
    }
...
void executeFirstStraightMovement() {
    ...
    //Determine: Is there a staircase directly in front of ?
     bool stairsAhead = isStairsNotWall();
     if (stairsAhead) {
         stopMotors();
         Serial.println("\n╔════════════════════════════════════╗");
         Serial.println("║    STAIRS DIRECTLY AHEAD         ║");
         Serial.println("║   (Straight / Right-angle staircase: 0 turns)");
         Serial.println("║   Skip turns, resume climbing      ║");
         Serial.println("╚════════════════════════════════════╝");

         platformNavState = NAV_IDLE;  //Go directly to the stairs and return to the stair-climbing process.
         stateStartTime = millis();
         return;
     }

        float avgWall = getAverageWallDistance();
        bool wallAhead =(avgWall < 90.0f &&avgWall>25.0f );  //  &&fabs(currentPitch) < 3.0f  Platform + Front Plane     !!!NEED TO TUNE

        if (wallAhead) {
            stopMotors();
            Serial.println("\n First straight movement complete");
            Serial.print("Avg wall distance: ");
            Serial.print(avgWall, 1);
            Serial.println(" cm -> treat as WALL, turning 90°");

            platformNavState = NAV_FIRST_TURN;  //     Entering the first 90-degree turn
            platformTurnCount = 1;
            platformTurnStartTime = millis();
            platformTurnYaw = 0.0f;
            stateStartTime = millis();
            return;
        }
...
void executeFirstTurn() {
    ...
    float turnedAngle = fabs(platformTurnYaw);
    ...
    if (turnedAngle >= (PLATFORM_TURN_ANGLE - PLATFORM_TURN_TOLERANCE)) {
        ...
        platformNavState = NAV_STAIR_OR_WALL_CHECK;
        stateStartTime = millis();
        platformTurnYaw = 0.0f;
        return; 
    }
...
void executeSecondTurn() {
    ...
    if (turnedAngle >= (PLATFORM_TURN_ANGLE - PLATFORM_TURN_TOLERANCE)) {
        ...
        Serial.println("  Proceeding to Stage 7: Next Stair Detection");
...
void logNavigationState() {
    ...
    switch (platformNavState) {
        ...
        case NAV_FIRST_TURN:
            ...
            break;
        case NAV_STAIR_OR_WALL_CHECK:
            Serial.print(" Check Stair Or Wall ");
            break;
        case NAV_APPROACH_WALL:
...
bool isStairsNotWall2() {
 ...
 // ============================================================================
// STAGE: AFTER FIRST TURN, CHECK IF AHEAD IS STAIRS OR WALL
// ============================================================================

void checkStairOrWall() {
    unsigned long elapsed = millis() - stateStartTime;

    //  Move forward slowly, a little at a time, to allow the ultrasound waves to have a more stable echo.
    if (elapsed < 500) {
        setMotorDirection(true);
        setMotorSpeed(60, 60);
        Serial.print(" After-turn probing... ");
    ...
    bool stairsAhead = isStairsNotWall();//isStairsNotWall2();

    if (stairsAhead) {
        //   L-shaped staircase: Platform -> Straight ahead -> Wall -> 90° -> Staircase (1 turn)
        Serial.println("\n╔════════════════════════════════════╗");
        Serial.println("║    STAIRS AFTER FIRST TURN       ║");
        Serial.println("║   (L-shaped: total 1 turn)        ║");
        Serial.println("╚════════════════════════════════════╝");

        platformNavState = NAV_IDLE;  // Go directly to the stairs & climb back up the stairs
        stateStartTime = millis();
    } else {
        // U-shaped staircase: Platform -> Straight -> Wall -> 90° -> Wall -> 90° -> Staircase (2 turns)
        Serial.println("\n╔════════════════════════════════════╗");
        Serial.println("║   WALL AFTER FIRST TURN            ║");
        Serial.println("║   Proceed to approach wall (U-type)║");
        Serial.println("╚════════════════════════════════════╝");

        platformNavState = NAV_APPROACH_WALL;     //   Entering the wall-adjacent phase, then turning a second time.
        stateStartTime = millis();
    }
}
